...
export function LoansClient({ mode }: { mode: "current" | "history" }) {
  ...
  return <>
    ...
    <section className="panel">{loading ? <div className="page-loading compact"><span className="auth-spinner"/><p>Checking your shelves…</p></div> : visibleLoans.length === 0 ? <EmptyState title="Your shelf is empty" copy="Borrow a book from the collection and it will appear here." action={<Link href="/catalog" className="button button-primary">Find a book</Link>}/> : <div className="loan-list">{visibleLoans.map((loan, index) => { const book = books[loan.bookId]; const readable = book?.digitalAccess === "PUBLIC_DOMAIN"; return <article className="loan-row loan-row-detailed" key={loan.loanId}>{book ? <BookCover book={book} index={index}/> : <div className="book-cover cover-navy"><strong>{loan.bookTitle}</strong></div>}<div className="loan-main"><span className="eyebrow">{book?.category ?? "Library book"}</span><h3>{loan.bookTitle}</h3><p>{book?.author ?? ""}</p></div><div className="loan-date"><small>Borrowed</small><strong>{formatLibraryDate(loan.borrowedAt)}</strong></div><div className="loan-date"><small>Due</small><strong>{formatLibraryDate(loan.dueDate)}</strong></div><Status status={loan.status}/><div className="loan-actions">{readable && <Link href={`/reader?id=${encodeURIComponent(loan.bookId)}`} className="button button-dark">Read</Link>}<button disabled={pendingLoan === loan.loanId} onClick={() => returnBook(loan)} className="button button-quiet">{pendingLoan === loan.loanId ? "Returning…" : "Return book"}</button></div></article>; })}</div>}</section>
    <div className="page-note"><Icon name="book"/><p><strong>Want to read in the app?</strong> Digital reading is available for selected public-domain classics.</p><Link href="/read">Read online</Link></div>
  </>;
}
